...
total="$(count_dns)"
printf "${C_WHITE}Проверяю %s DNS/DoH параллельно...${C_NC} ${C_YELLOW}(может занять до минуты)${C_NC}\n" "$total"
n=0
...
printf "${C_YELLOW}⏳ Идёт параллельный перебор всех DoH-эндпоинтов — это не самый быстрый шаг, подождите до минуты.${C_NC}\n"
